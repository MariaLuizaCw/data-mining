...
def get_bus_train_data_datahora(conn, linha):
    ...
    query = f'''select
                a.*,
                null  as latitude_resposta,
                null   as longitude_resposta,
                null  as filename_resposta
                from public.vehicle_tracking_teste_datahora  a
                where
                linha = '{linha}'
            '''
...
def get_bus_train_data_latlong(conn, linha):
    ...
    query = f'''select
                a.*,
                    null as datahora_epoch_resposta,
                    null as filename_resposta
                from public.vehicle_tracking_teste_latlong a 
                where
                linha = '{linha}'
            '''
...
def get_bus_train_data(conn, linha):
    query = f'''select 
                ...
                from vehicle_tracking_teste_base where 
                linha = '{linha}'
            '''
...
insert_query_datahora = '''
    INSERT INTO vehicle_tracking_pred_datahora_teste (id, filename, datahora_epoch_pred, datahora_epoch_resposta)
    VALUES %s
    '''


insert_query_latlong = '''
    INSERT INTO vehicle_tracking_pred_latlong_teste (id, filename, latitude_pred, longitude_pred, latitude_resposta, longitude_resposta)
    VALUES %s
    '''
